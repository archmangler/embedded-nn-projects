...
void matrix_vector_multiply(double * input_vector, int INPUT_LEN,double * output_vector, int OUTPUT_LEN,double weight_matrix[OUTPUT_LEN][INPUT_LEN]){
...
//2d Matrix normalization is needed here since we're dealing with matrix not vector as we were earlier.
...
//initialize the weights vector with random values.
